...
assert.match(approvalRisk, /android_eyevue_look"\) return toolArgs\?\.lookAgain === true/);
assert.match(approvalRisk, /android_eyevue_command[\s\S]*EYEVUE_CAPTURE_COMMANDS/);
assert.match(approvalRisk, /isEyevueCaptureAction\(toolName, toolArgs\)\) return true/);
