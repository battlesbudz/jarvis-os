...
-   **Mission Control UI:** Five-section dashboard with a top segment control (Tasks · Calendar · Projects · Memory · Visual). Green (`#22c55e`) is the primary accent, purple (`#a855f7`) is the selection/highlight colour, background is near-black (`#09090f`). Header shows "JARVIS COMMAND" + pixel-art VisionSprite character + live PRIME ONLINE status dot (polls `/api/agents` every 30s). Tasks tab = Kanban (Backlog / In Progress / Done) from `/api/jarvis/scheduled-tasks`. Calendar tab = scheduler card list combining user tasks and system schedule. Memory tab = date-grouped journal from `/api/memories` with search and category filter chips. Projects and Visual tabs show placeholder cards. Sub-screens live in `components/missionControl/`, VisionSprite in `components/VisionSprite.tsx`.
...
- **DASHBOARD_SECRET**: Env var (`jarvis-command-2026`) set in shared env + `dashboard/.env.local`. Bypass in `server/auth.ts → authMiddleware` allows requests with this token.
