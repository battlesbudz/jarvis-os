...
- **YouTube transcript system (April 2026)** — `get_youtube_transcript` tool in `server/agent/tools/youtubeTranscript.ts`. Six-strategy cascade: (1) InnerTube API (TVHTML5→iOS→ANDROID), (2) yt-dlp subtitle extraction (`server/lib/transcriptCache.ts`), (3) timedtext XML, (4) youtube-transcript library, (5) Playwright browser fallback, (6) local worker. Playlist URLs rejected with a clear message. Long transcripts (>40k chars, ≈1h+ video) are sent as a `.txt` file attachment via `pendingAttachments` instead of flooding the chat. `server/lib/localWorkerQueue.ts` provides an in-memory job queue for the local worker; API routes at `GET /api/local-worker/token`, `POST /api/local-worker/heartbeat`, `GET /api/local-worker/jobs/next`, `POST /api/local-worker/jobs/:id/complete|fail`. Standalone local worker script at `scripts/jarvis-local-worker.js` — user runs this on their PC with yt-dlp installed.
