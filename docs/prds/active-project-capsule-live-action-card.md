...
Every transition to `approved`, regardless of producer, calls one owner-aware `decideApprovalGate` service. This includes the Live Action Card, agent API, deliverable review, gateway, Discord, Telegram, Slack, WhatsApp, webchat, inbox triage, scheduled/internal automation, and creation-time policy auto-approval. Existing route/channel handlers and internal producers become thin authorized adapters and must not call `approveGate` or write `status = approved` directly. Every approval request carries a stable caller-supplied `toolExecutionId` derived from the logical workflow/run/tool-call identity, including across process and job retries. At first creation the dispatcher stores an immutable `operationDigest` computed from a versioned canonical serialization of tool name, effect scope, and complete arguments; only the digest and existing redacted review summary persist in the projection-facing approval fields. Executable arguments persist separately as an encrypted, owner- and user-scoped operation envelope (or durable encrypted payload reference) with least-privilege access and audit controls. The operation envelope is part of the same database transaction as the gate and continuation intent; an external encrypted payload store may be used only through a recoverable staged-write protocol that durably stages the payload before the transaction publishes its reference, verifies the staged object during commit, and garbage-collects unreferenced staged objects. Envelope retention is lifecycle-coupled rather than an independent TTL: it remains readable through every pending gate and every nonterminal continuation/reconciliation state, including `outcome_unknown`, and may be deleted only after the gate expires without approval or the continuation reaches a safely reconciled terminal outcome. The dispatcher decrypts the committed envelope only at execution time, recomputes and verifies `operationDigest` immediately before dispatch, and fails closed on missing, expired, or mismatched payloads; logs, Live Actions, diagnostics, and outcome records never expose the raw envelope. The dispatcher idempotently upserts the gate, operation envelope/reference, decision receipt, continuation claim, and provider key by `(userId, toolExecutionId)`, but returns the existing receipt only when `operationDigest` matches exactly. A mismatch is a conflict: it cannot reuse the prior approval or dispatch an effect and must receive a new logical execution ID and approval decision. The provider idempotency key incorporates the execution ID and digest; a random gate ID is only a surrogate and never the effect idempotency key. Creation-time auto-approval uses this dispatcher entry point to create or reuse the envelope, approved gate, decision receipt, and continuation intent in one transaction before returning to execution. The service preserves the current gate/receipt authorization and policy semantics, resolves the canonical continuation owner (top-level job, direct email/Jarvis decision, Agent SDK run, or an in-job checkpoint), and returns one idempotent decision receipt.

For an approval, a database transaction records the gate decision, executable operation envelope/reference, immutable digest, and a uniquely keyed continuation outbox/claim before the gate can become approved. A worker dispatches that durable intent to the canonical owner and writes the handoff/outcome protocol below. For rejection, the same transaction records the terminal rejection outcome. If the process exits after commit, reconciliation resumes the outbox with its committed executable payload; there is never an approved gate without both durable continuation intent and its digest-bound operation envelope. Contract tests invoke every route/channel and internal producer, including inbox triage and policy auto-approval, and prove identical owner dispatch, duplicate-decision idempotency, rejection behavior, and recovery at each crash boundary: before envelope staging, after staging but before database commit, after commit but before continuation dispatch, and after dispatch as covered by the provider protocol below. A static ownership test prevents new direct approved-state writes or `approveGate` calls outside the canonical dispatcher/storage implementation.
...
For approval continuations that do not create an agent job, the canonical approval owner must persist an idempotent continuation outcome before the HTTP decision route returns. The durable record is keyed by gate ID and records an allowlisted owner type/reference plus `started | succeeded | failed | rejected | outcome_unknown | superseded_unknown`, timestamps, and a safe failure category; it never stores raw tool arguments or secrets. `handleJarvisApprovalDecision`, direct-email continuation, and Agent SDK resumption must write `started` and then a terminal outcome or durable `outcome_unknown` (or atomically update an equivalent existing receipt). The approval projector keeps the same lineage, shows running after `started`, derives succeeded, failed, or cancelled/rejected only from those persisted outcomes, maps durable `outcome_unknown` to `waiting_user` with an explicit “result could not be verified” warning, and maps terminal `superseded_unknown` to `expired` with a permanent audit note that the original effect was never verified. A transient callback result or `approved` gate status alone is insufficient. Before any external effect, the owner atomically claims the continuation with a deterministic idempotency key derived from the gate and operation. The claim distinguishes `claimed` (provider dispatch has not begun) from `dispatching`; the owner writes `dispatching` immediately before the provider call, passes the key to providers that support idempotency, and persists the provider operation receipt/reference before recording the terminal outcome. Reconciliation resolves a stranded claim with a provider-specific three-state lookup: `found`, `conclusively_absent`, or `indeterminate`. It waits the documented provider consistency window and required repeated checks before accepting absence. A `claimed` lease that expired before `dispatching` may be safely reclaimed; a `dispatching` operation may be retried only when the provider guarantees idempotency for the same key. `conclusively_absent` may return to a retryable claimed state, while `indeterminate` persists `outcome_unknown` and is never replayed automatically. From `outcome_unknown`, the user may mark the result verified-succeeded or verified-failed. If the user explicitly requests replacement work, the client/caller supplies only a stable `replacementRequestId` for that logical request plus the complete replacement operation; the server canonicalizes that submitted payload and computes the replacement digest itself before any lookup, or verifies any transport-supplied digest against the server-computed value and rejects a mismatch. One transaction transitions the original record to terminal `superseded_unknown` and creates a replacement outbox uniquely keyed by `(userId, originalToolExecutionId, replacementRequestId)` containing the new `toolExecutionId`, server-computed replacement digest, digest-bound encrypted envelope/reference, applicable approval intent, and link to the superseded record. A repeated `replacementRequestId` reuses the intent only when the newly server-computed incoming digest matches exactly; a mismatch returns a conflict without dispatch or stale-envelope reuse and requires a new logical replacement request ID. The original preserves its digest, receipt references, warning, and audit history without asserting success or failure, and its encrypted executable envelope becomes deletion-eligible only after the replacement outbox and envelope/reference commit successfully. A worker then creates or resumes the separate replacement operation from that durable outbox; it never reuses the original effect identity. Every choice is authenticated, audited, and persisted. A direct-email adapter uses a deterministic message identity and Sent lookup, but a not-found result remains indeterminate unless the provider contract makes absence conclusive. Owners with neither provider idempotency nor receipt lookup also use `outcome_unknown`. Agent SDK resumption additionally writes a durable per-tool execution ledger keyed by gate/run/tool-call ID and applies this same claim, key, receipt, and reconciliation protocol before replaying any effectful tool; the SDK cursor alone is not an idempotency boundary. Reconciliation must repair the Live Action from this durable owner record after reconnect, and adapter tests cover success, rejection, continuation failure, durable unknown-state recovery and user resolution, atomic unknown-effect supersession and replacement creation, exact duplicate replacement reuse, forged-digest and changed-payload conflicts, envelope cleanup eligibility, lifecycle-coupled envelope retention, a crash after supersession commit but before replacement dispatch, and a crash after provider acceptance but before terminal persistence for every non-job owner path.
