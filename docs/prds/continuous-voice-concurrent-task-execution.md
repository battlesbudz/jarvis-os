...
17. A linked operation fails after an earlier forward step produced an effect; Jarvis fences all remaining unstarted forward work but preserves applicable predeclared compensation, keeps the task nonterminal while any compensation is pending or `reconciliation_required`, permits a bounded retry only after reconciliation proves no effect and before `compensationExpiresAt`, and reports terminal failure only after compensation reaches a proven outcome or canonical expiry.
