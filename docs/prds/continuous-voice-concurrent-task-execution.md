...
- The recognizer preserves confidence and N-best alternative evidence and forwards material-slot uncertainty to Trusted Execution. A nonempty best hypothesis is not sufficient authority for a material side effect unless its route supplies validated confidence evidence and that evidence meets the route threshold without N-best disagreement on a target, recipient, account, repository, environment, item, amount, price ceiling, quantity, date, time, timezone, attendee, duration, scope, or choice. If the route lacks that validated evidence, confidence is below threshold, or alternatives disagree, the material field uses the single focused `clarify` path before authority issuance; a route unable to provide or obtain that evidence fails closed for the action.
...
- Every task terminal transition acquires that lifecycle lock, closes amendment admission, and snapshots all linked authorities after winning the lock. Task success requires every required original and amendment-linked operation in that snapshot to have a successful terminal outcome; pending, failed, cancelled, or reconciliation-required amendment work prevents a false `completed` result. Before task failure becomes terminal, the same transaction closes forward admission and cancels or fences every unstarted forward step across all remaining linked nonterminal authorities while preserving applicable predeclared compensation for effects already produced. The task remains nonterminal through that bounded compensation phase and becomes failed only after every required compensation has a proven terminal success or failure. A compensation attempt in `reconciliation_required` remains part of the nonterminal compensating task while Trusted Execution reconciles it and, after proof of no effect, may perform its bounded retry before `compensationExpiresAt`; only a proven outcome or expiry under the canonical compensation contract permits terminal task failure. An already-started uncertain forward outcome becomes `reconciliation_required`, reserves its potentially applicable compensation in `awaiting_effect_reconciliation`, and keeps the parent task nonterminal until effect reconciliation proves no effect, confirms an effect and its resulting bounded compensation settles, or canonical recovery expiry is reached. Recovery guidance surfaces throughout; neither reconciliation nor compensation may resume fenced forward work.
...
- Recognition confidence is low, alternatives disagree, or the route supplies no validated confidence evidence for a material side-effect field: preserve available alternatives as transient classifier evidence and ask one focused clarification before issuing authority; do not execute the best hypothesis, and fail closed if the missing evidence cannot be obtained.
...
- Provider or tool outcome is uncertain: quarantine/reconcile through Trusted Execution rather than retrying blindly; keep the parent task nonterminal and any potentially applicable compensation reserved until effect reconciliation or canonical recovery expiry.
...
5. after the Trusted Execution PR 5 canonical-policy parity gate passes and the repository's required explicit high-trust human review approves the changes to approvals, authentication, daemon control, and deployment behavior, enable concurrent task handoff and progress conversation;
...
16. Recognition produces a low-confidence best hypothesis, competing alternatives, or no validated confidence evidence for a material recipient, amount, repository, target, or other side-effect field; Jarvis issues no authority for that hypothesis, asks one focused clarification in the voice conversation, and fails closed for the action if validated evidence cannot be obtained.
17. A linked operation fails after an earlier forward step produced or may have produced an effect; Jarvis fences all remaining unstarted forward work, reserves potentially applicable compensation while the forward attempt is `reconciliation_required`, and keeps the task nonterminal until effect reconciliation proves no effect or confirms the effect and bounded compensation settles. An uncertain compensation attempt likewise remains parent-blocking, permits a bounded retry only after reconciliation proves no effect and before `compensationExpiresAt`, and allows terminal failure only after a proven outcome or canonical recovery expiry.
...
- Before action-first voice flags enable, the approval, authentication, daemon-control, and deployment changes have received the repository's explicit high-trust human review required by root `AGENTS.md`; a clean automated Codex review does not substitute for this gate.
