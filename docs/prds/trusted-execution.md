...
interface ExecutionAuthority {
  ...
  compensationReason?: "forward_failure" | "workflow_incomplete_at_expiry"; // required while compensating
...
- Parent active/compensating/completed/failed/cancelled/expired state, forward `expiresAt`, fixed `compensationExpiresAt`, durable open/closed forward-admission state and closure timestamp, persisted compensation reason while compensating, terminal reason/recovery reference, and reconciliation none/required/resolved state; per-step pending/consuming/consumed/retryable-failed/failed/cancelled/skipped/reconciliation-required state
