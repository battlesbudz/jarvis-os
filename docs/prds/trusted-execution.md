...
- `trusted_execution_rollback_started`, `rollback_completed`, `rollback_failed`
...
| Definitive child failure leaves parent active | Workflow hangs until expiry or later steps start after failure | Atomic failed-parent transition, cancel unstarted manifest entries, quarantine uncertain steps, audit-only reconciliation with no revival |
