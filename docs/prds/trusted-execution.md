...
| TE-028 | Admission and compensation expiry | Original issuance fixes forward `expiresAt` and policy-capped `compensationExpiresAt`. Normal expiry atomically closes new forward admission but preserves an on-time started/uncertain attempt until its result can select completion, failure, or bounded compensation; only rollback for a boundary crossed before normal expiry may use the recovery window, and neither deadline revives forward authority |
...
- Durable child execution-step records unique by `(authorityId, stepKey)`, one for every closed-manifest entry, each with resolved action, target fingerprint, independent idempotency key, immutable `role` (`forward` or `compensation`), immutable `compensatesStepKeys` for compensation, child attempt limit bounded by the parent's `maxAttemptsPerStep`, durable attempt count/current attempt, consumption/reconciliation state, result/recovery reference, and timestamps
...
- When retry policy/attempts are exhausted or a forward step is definitively unrecoverable, one transaction marks the step failed, stores sanitized failure/recovery references, cancels every unstarted forward step as `upstream_failure`, and either terminally fails the parent or moves it to `compensating` for only the applicable compensation children already frozen into the closed manifest. Nonapplicable compensation children become `skipped`; uncertain already-started steps block dependent compensation pending reconciliation. Compensation and reconciliation cannot revive or resume forward work, and the final parent outcome remains `failed`.
- At forward `expiresAt`, atomically close forward admission, fence/cancel pre-effect attempts and unstarted forward children, safely release their proven-no-effect allocations, and preserve allocations for on-time started/uncertain attempts plus potentially applicable compensation as charged and step-owned. Do not terminalize or settle the parent while an on-time outcome remains unresolved; when no such unresolved attempt exists—including when earlier crossed attempts are already consumed/resolved—atomically complete when still possible or terminally expire/fail and settle. A later result transaction likewise completes/expires/fails or enters `compensating` within the fixed recovery window.
...
- Forward-expiry race, compensation-expiry, cancellation, attempt-limit, and child-delegation tests proving `expiresAt` atomically closes new forward admission, fences pre-effect attempts, preserves only an on-time unresolved started/uncertain attempt and its allocations until result/reconciliation, terminally expires when all crossed attempts were already resolved but later forward work is cancelled, permits only applicable predeclared rollback until fixed `compensationExpiresAt`, and blocks late forward crossings and post-recovery-deadline rollback without revival
...
| Forward result arrives after normal expiry | Premature expiry strands recovery, or historical completion prevents terminal settlement forever | Treat normal expiry as an atomic forward-admission close; preserve only on-time unresolved attempts and potential compensation, then terminally settle once none remain; never let a historical resolved attempt extend authority |
