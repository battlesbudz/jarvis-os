...
| TE-028 | Admission and compensation expiry | Original issuance fixes forward `expiresAt` and policy-capped `compensationExpiresAt`. Normal expiry atomically closes new forward admission; it preserves an on-time unresolved attempt and never terminalizes an already-`compensating` parent. Only rollback for a boundary crossed before normal expiry may use the recovery window, and neither deadline revives forward authority |
...
| Forward result or compensation overlaps normal expiry | Premature expiry strands recovery, while historical completion can prevent settlement forever | Close forward admission atomically; preserve only on-time unresolved attempts; terminally expire an eligible active parent once none remain; never terminalize an already-compensating parent before its recovery deadline/control event |
