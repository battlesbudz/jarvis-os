...
- Forward-expiry race, compensation-expiry, cancellation, attempt-limit, and child-delegation tests proving `expiresAt` atomically closes new forward admission, fences pre-effect attempts, preserves an on-time started/uncertain attempt and its allocations until result/reconciliation, permits only its predeclared rollback until fixed `compensationExpiresAt`, and blocks late forward crossings and post-recovery-deadline rollback without revival
...
| Forward result arrives after normal expiry | Premature terminal expiry strands reconciliation or declared rollback | Treat normal expiry as an atomic forward-admission close; preserve on-time started/uncertain attempts and potential compensation until result or fixed recovery deadline; never extend forward authority |
