...
| Forward completion or compensation overlaps a deadline | Success can be mislabeled expired, or rollback can be skipped/terminalized before its trigger resolves | At forward expiry complete and skip rollback only after durable forward success; otherwise retain potential compensation, preserve on-time unresolved attempts, and keep compensation/reconciliation nonterminal until resolution or fixed recovery deadline |
