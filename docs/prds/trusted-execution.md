...
interface ExecutionAuthority {
  ...
  issuedAt: string;
  expiresAt: string; // deadline for starting forward steps
...
- Forward authority expires at `expiresAt`. A forward step that crossed its boundary before that deadline may activate only its predeclared rollback under nonterminal `compensating` until the separately issued, policy-capped `compensationExpiresAt`; the recovery deadline cannot revive or extend forward work. Successful closed manifests complete with required forward entries `consumed` and compensation entries `skipped`. Failed workflows become terminal after bounded compensation resolves, is blocked, or expires. Terminal parents never start another step, and standing-parent terminal transitions atomically settle every allocation.
...
| TE-028 | Bounded compensation expiry | Original issuance fixes a policy-capped `compensationExpiresAt`; only rollback for a forward boundary crossed before `expiresAt` may use it, and recovery-window expiry cancels unstarted compensation without extending or reviving forward authority |
...
- Parent active/compensating/completed/failed/cancelled/expired state, forward `expiresAt`, fixed `compensationExpiresAt`, terminal reason/recovery reference, and reconciliation none/required/resolved state; per-step pending/consuming/consumed/retryable-failed/failed/cancelled/skipped/reconciliation-required state
...
- Global/per-user disable transactions increment monotonic revocation epochs and logically invalidate/cancel every unstarted authority and child step with an older snapshot. Final tool/step consumption atomically verifies enabled state, both epoch snapshots, parent authority state, child step state, and applicable grant head before crossing that step's irreversible boundary.
...
- Forward-expiry, compensation-expiry, cancellation, attempt-limit, and child-delegation tests proving a forward boundary crossed before `expiresAt` may execute only its predeclared rollback until fixed `compensationExpiresAt`, while late forward starts and post-recovery-deadline rollback are blocked without revival
...
| Forward failure arrives after authority expiry | Declared rollback is stranded after a long-running operation | Fix a separate policy-capped compensation deadline at issuance; allow only rollback for forward boundaries crossed before normal expiry; never extend forward authority |
