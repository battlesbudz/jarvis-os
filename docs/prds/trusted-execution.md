...
| TE-028 | Admission and compensation expiry | Original issuance fixes forward `expiresAt` and policy-capped `compensationExpiresAt`. Normal expiry atomically closes new forward admission, derives successful closed-manifest completion before expiry, preserves an on-time unresolved attempt, and never terminalizes an already-`compensating` parent. Compensation reconciliation stays nonterminal until resolved or recovery expiry; neither deadline revives forward authority |
...
- Forward-expiry race, compensation-expiry, cancellation, attempt-limit, and child-delegation tests proving `expiresAt` atomically closes new forward admission, derives completion before expiry when all required forward steps are consumed and rollback is skipped, fences pre-effect attempts otherwise, preserves only an on-time unresolved started/uncertain attempt and its allocations until result/reconciliation, terminally expires an `active` parent when all crossed attempts were already resolved but later forward work is cancelled, never terminalizes an already-`compensating` parent, permits only applicable predeclared rollback until fixed `compensationExpiresAt`, and blocks late forward crossings and post-recovery-deadline rollback without revival
...
| Forward completion or compensation overlaps a deadline | Success can be mislabeled expired, or uncertain rollback can become terminal before safe retry | At forward expiry derive completion first, preserve only on-time unresolved attempts, and never terminalize an already-compensating parent; keep compensation reconciliation nonterminal until resolution or fixed recovery deadline |
