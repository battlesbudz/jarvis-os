...
```mermaid
flowchart LR
  Intent["User intent"] --> Surfaces["Expo app, dashboard, channel adapters"]
  Surfaces --> Runtime["Express server + route modules"]
  Runtime --> Router["Provider routing + context assembly"]
  Router --> Harness["Agent harness"]
  Harness --> Memory["Memory OS, SOUL, G-Brain"]
  Harness --> Jobs["Agent jobs, workers, deliverables"]
  Harness --> Approvals["Approval gates + receipts"]
  Harness --> Tools["Tools + integrations"]
  Tools --> Connectors["Desktop connector + Android daemon"]
  Runtime --> Database["PostgreSQL via Drizzle"]
  Memory --> Database
  Jobs --> Database
  Approvals --> Database
```
...
| Dashboard | `dashboard/` | Next.js mission-control surface for tasks, projects, memory, calendar, and visual office views. |
| Connectors | `daemon/`, `desktop-connector/`, `android-daemon/` | Optional local-device bridges paired back to the server. |
| Shared contract | `shared/` | Drizzle schema and shared runtime models. |
| Workspace content | `workspaces/` | Optional local operating documents used by a deployment. Public examples should stay generic. |
...
- `workspaces/` can hold deployment-specific operating context. Do not place private workspace content in public-facing docs.
