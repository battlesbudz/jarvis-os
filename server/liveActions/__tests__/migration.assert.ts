...
assert.match(jobQueue, /appendWorkerEventToJob[\s\S]*db\.transaction\(async \(tx\)[\s\S]*\.for\("update"\)/);
assert.match(jobQueue, /const inputPatch = \{[\s\S]*workerRuntime: nextInput\.workerRuntime,[\s\S]*workerType: nextInput\.workerType/);
