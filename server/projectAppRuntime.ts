...
function injectBridge(html: string, permissions: JarvisAppManifest["permissions"]): string {
  ...
  const script = bridgeScript(permissions);
  const securedHtml = /<head\b[^>]*>/i.test(html)
    ? html.replace(/<head\b[^>]*>/i, (head) => `${head}${csp}`)
    : `${csp}${html}`;
  return /<\/body\s*>/i.test(securedHtml) ? securedHtml.replace(/<\/body\s*>/i, `${script}</body>`) : `${securedHtml}${script}`;
}
