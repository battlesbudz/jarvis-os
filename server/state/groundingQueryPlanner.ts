...
export function classifyGroundingIntent(requestText: string): GroundingIntent {
  ...
  if (/\b(?:what do you know about me|(?:tell|show) me what you know about me|tell me (?:one|a) (?:thing|fact) (?:that )?you (?:know|remember) about me|what do you remember about me|whats in my memory|what is in my memory|show my memories|list my memories)\b/.test(text) ||
...
export function shouldExcludeTaskGuidanceForRecall(
  ...
  const leadingScope = normalizedQuery.match(
    /^(?:for|about|regarding|on|with|concerning|around|related to|relating to|when it comes to|in relation to|as to)\s+(.+?)(?:[,;:]\s*|\s+(?=(?:what|which|could|can|would|tell|show)\b)).*\b(?:preferences?|values?)\b/,
  )?.[1];
