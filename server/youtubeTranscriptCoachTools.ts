...
export function buildYoutubeTranscriptCoachTools(): CoachTool[] {
  return [
    coachFunctionTool({
        ...
        description: "Fetch the COMPLETE transcript/captions of a YouTube video server-side â€” returns the full text with no truncation. Use this INSTEAD of navigating YouTube's transcript UI on the phone.\n\nINTERNAL PIPELINE â€” this tool automatically tries multiple methods in order:\n  Phase 0:   Gemini multimodal â€” feeds the video URL directly to Gemini AI\n  Phase 0.5: Supadata â€” a cloud transcript API (supadata.ai) that bypasses YouTube's IP blocks. Uses mode=auto: tries native captions first, then AI-generates a transcript if no captions exist. This costs Supadata credits for AI generation.\n  Phase 1-4: YouTube InnerTube API, yt-dlp subtitles, timedtext, youtube-transcript library\n  Phase 5:   Whisper audio transcription (downloads audio via yt-dlp, then transcribes)\n  Phase 6:   Tavily web search fallback (last resort â€” summaries, not a real transcript)\n\nThe 'via X' label in the result (e.g. 'via Supadata', 'via YouTube captions', 'via Whisper (audio)') tells you which phase succeeded.",
        parameters: {
          type: "object",
          properties: {
    ...
    coachFunctionTool({
        ...
        description: "Fetch a YouTube transcript using only native YouTube captions â€” no AI, no credits charged. Tries InnerTube, yt-dlp subtitles, timedtext, and the youtube-transcript library. Fast, but only works if the video actually has captions. Use when the user explicitly wants captions-only (no AI generation).",
        parameters: {
          type: "object",
          properties: {
            videoId: { type: "string", description: "YouTube video ID (11 characters, e.g. 'dQw4w9WgXcQ') or full YouTube URL." },
