...
const TOOL_AWARE_RULES: ToolAwareRule[] = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(?:[$\w.\/&-]+\s+){1,6}(?:events?|headlines?|videos?|uploads?|posts?|information|info|data|prices?|scores?|results?|versions?|releases?|rates?|values?|rankings?|standings?|polls?|odds?|availability|status|population|counts?|totals?)\s+(?:today|currently|recently|latest|now|right\s+now)\b/i,
      /\b(?:events?|headlines?|videos?|uploads?|posts?|information|info|data|prices?|scores?|results?|versions?|releases?|rates?|values?|rankings?|standings?|polls?|odds?|availability|status|population|counts?|totals?)\s+(?:of|for|from)\s+(?:[$\w.\/&-]+\s+){1,8}(?:today|currently|recently|latest|now|right\s+now)\b/i,
