...
export class McpClient {
  ...
  private async _httpRequest(method: string, params: unknown): Promise<JsonRpcResponse> {
    ...
    if (ct.includes("text/event-stream")) {
      // SSE: scan for the event whose id matches ours
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<McpToolResult> {
    ...
    let res: JsonRpcResponse;
    if (this.transport.type === "stdio") {
      res = await this._stdioRequest("tools/call", { name, arguments: args });
    } else {
      res = await this._httpRequest("tools/call", { name, arguments: args });
    }
    if (res.error) {
