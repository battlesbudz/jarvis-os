...
- android_eyevue_command: control eyeVue media using command battery, storage, photo, video_start, video_stop, audio_start, or audio_stop. Photo waits for the AA15 image and leaves the original on the glasses. Ask where to save photos/finished recordings; do not copy them to Gallery or MemoryOS without an explicit answer.
- android_eyevue_look: answer a visual question with local Phone Gemma. Reuses the current eyeVue image by default; set lookAgain=true only when the user says “look again.” It never uses cloud vision. If local vision fails, ask whether to save the image and ask separately before cloud fallback.
