...
assert.match(
  inAppSearchSource,
  /let resultsLoaded = resumeFromStep === 5[\s\S]*await isVerifiedResultsState\(screenRaw, false\)[\s\S]*if \(resultsLoaded\)[\s\S]*outcome: "already_loaded"[\s\S]*no submit action dispatched[\s\S]*async function isVerifiedResultsState[\s\S]*screenMatchesResolvedApp\(raw\)[\s\S]*type: "android_get_focused_field"[\s\S]*!extractFocusedFieldText\(focus\.data\)\.focused[\s\S]*isVerifiedResultsState\(afterSearchRaw\)[\s\S]*isVerifiedResultsState\(retryRaw\)/,
