...
assert.equal(androidApprovalGateStart, -1, "ordinary Android runtime actions must not be blanket-gated as high stakes");
assert.equal(androidApprovalGateSource, "", "the blanket Android approval gate should be removed");
assert.doesNotMatch(routesSource, /androidSubmitApprovalRequired/);
assert.doesNotMatch(routesSource, /buildAndroidSubmitConfirmationPreview/);
assert.doesNotMatch(routesSource, /isAndroidSubmitCapableAction/);
assert.doesNotMatch(routesSource, /androidRouteApprovalRequired \|\|/);
...
assert.match(
  inAppSearchSource,
  /fieldText\.trim\(\) === searchQuery\.trim\(\)[\s\S]*visibleValues\.push\(\.\.\.compactScreen\.text[\s\S]*clickable\.label[\s\S]*normalizedQuery\.length > 0[\s\S]*visibleValues\.some[\s\S]*normalizeVisibleText\(value\)[\s\S]*` \$\{normalizedQuery\} `\)[\s\S]*focusedFieldTextMatches === null && screenContainsQuery/,
  "in-app search must reject appended stale text and require the complete bounded query from visible compact-screen values only",
);
assert.match(
  inAppSearchSource,
...
assert.doesNotMatch(approvalToolRiskSource.slice(highRiskToolsStart, irreversibleToolsStart), /\.\.\.ANDROID_PHONE_RUNTIME_TOOL_NAMES/);
assert.doesNotMatch(approvalToolRiskSource.slice(irreversibleToolsStart), /\.\.\.ANDROID_PHONE_RUNTIME_TOOL_NAMES/);
