...
assert.match(nativeInferenceEngine, /warmEngineReleaseRequested\.set\(true\)[\s\S]*releaseWarmEngineIfIdle\(\)/);
assert.match(nativeInferenceEngine, /operationAdmission\.releaseGeneration\(requestId\)[\s\S]*releaseWarmEngineIfIdle\(\)/);
