...
function fallbackRuntimeStateCardPrompt(
  ...
  return [
    ...
    `- Active model: ${entry.providerName}:${entry.model}`,
    `- Current context: ${currentContextForProviderRoute(entry, params)}`,
    "",
    "Active Task State:",
...
async function attachProviderRuntimeStateCard(
  ...
  try {
    const prompt = await buildRuntimeStateCardPrompt({
      ...
      activeModel: `${primary.providerName}:${primary.model}`,
      currentContext: currentContextForProviderRoute(primary, params),
    ...
    return insertRuntimeStateCardMessage(
      routedMessages,
      fallbackRuntimeStateCardPrompt(params, primary),
    );
  }
}
