      ...
      switch (toolName) {
        ...
        case 'check_connections': {
          ...
          const daemonLabel = daemonOnline
            ? daemonParts.join(" | ")
            : `Android/Desktop Daemon: ✗ not connected — for Android device control, install/open the main Jarvis Android app, go to Profile → Android Device, tap Code, then tap Connect. The app uses the configured server URL automatically.`;
          const lines = [
  ...
  app.post("/api/coach/chat", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const daemonSection = daemonPaired
        ...
        : '⚠️ NO DAEMON CONNECTED. Do NOT call daemon_action — it will fail with "daemon not connected". If the user asks to control their phone or computer, tell them exactly this: "Your phone device control isn\'t connected. To fix it: (1) Install/open the main Jarvis Android app, (2) Go to Profile and scroll to Android Device, (3) Tap Code to generate a pairing code, (4) Tap Connect. The app uses the configured server URL automatically. The status dot should turn green within a few seconds." Do not attempt daemon_action until they confirm it\'s connected.';
