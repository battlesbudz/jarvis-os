      ...
      switch (toolName) {
        ...
        case 'check_connections': {
          ...
          const daemonLabel = daemonOnline
            ? daemonParts.join(" | ")
            : `Android/Desktop Daemon: ✗ not connected — user must open the main Jarvis Android app → Profile → Android Device → Get Pairing Code, enter server URL https://gameplanjarvisai.up.railway.app and the 8-character code, then tap Pair`;
          const lines = [
  ...
  app.post("/api/coach/chat", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const daemonSection = daemonPaired
        ...
        : '⚠️ NO DAEMON CONNECTED. Do NOT call daemon_action — it will fail with "daemon not connected". If the user asks to control their phone or computer, tell them exactly this: "Your phone device control isn\'t connected. To fix it: (1) Open the main Jarvis Android app → Profile → scroll to \'Android Device\' → tap \'Get Pairing Code\', (2) Make sure the Server URL is https://gameplanjarvisai.up.railway.app, (3) Enter the 8-character pairing code, (4) Tap Pair. The status dot should turn green within a few seconds." Do not attempt daemon_action until they confirm it\'s connected.';
