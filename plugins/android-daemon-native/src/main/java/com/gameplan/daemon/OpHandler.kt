...
object OpHandler {
        ...
        if (svc == null) {
            ...
            return OpResult(
                ...
                error = "Accessibility service is not running — the app could NOT be launched automatically. " +
                    ...
                    "To fix autonomous control: go to Settings > Accessibility > Installed Apps > Jarvis app and enable it."
            )
        }
        ...
        if (svc == null) {
            ...
            return OpResult(
                ...
                error = "Accessibility service is not running — the URL could NOT be opened automatically. " +
                    ...
                    "To fix: Settings > Accessibility > Installed Apps > Jarvis app and enable it."
            )
        }
    ...
    private fun handleNotificationsList(context: Context, op: JSONObject): OpResult {
        ...
        return OpResult(
            ...
            data = JSONObject()
                ...
                .put("notificationLastError", JarvisNotificationListener.lastError)
                .put("hint", when {
                    !permissionGranted -> "Grant notification access in Settings > Notifications > Device & App Notifications > Jarvis app"
    ...
    private fun handleNotificationOpen(context: Context, op: JSONObject): OpResult {
        ...
        val svc = JarvisAccessibilityService.instance
            ?: return OpResult(false, error = "Accessibility service is not running, so Jarvis cannot use the notification-shade fallback.")
        ...
        if (!preciseGranted && !coarseGranted) {
            return OpResult(
                ...
                error = "LOCATION_PERMISSION_REQUIRED: Location permission is not granted. " +
                    "Go to Settings → Apps → Jarvis app → Permissions → Location and select 'Allow all the time' or 'Allow only while using the app'."
            )
        }
        ...
        if (context.checkSelfPermission(Manifest.permission.SEND_SMS) != PackageManager.PERMISSION_GRANTED) {
            return OpResult(
                ...
                error = "SMS_PERMISSION_REQUIRED: SEND_SMS permission is not granted. " +
                    "Go to Settings → Apps → Jarvis app → Permissions → SMS and enable it."
            )
        }
