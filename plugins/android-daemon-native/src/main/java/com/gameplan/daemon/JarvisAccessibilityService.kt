...
class JarvisAccessibilityService : AccessibilityService() {
    ...
    fun openNotificationFromShade(query: String, appName: String? = null): NotificationShadeOpenResult {
        ...
        if (!performGlobalAction(GLOBAL_ACTION_NOTIFICATIONS)) {
            return NotificationShadeOpenResult(false, error = "Android did not open the notification shade.")
        }
        ...
        return NotificationShadeOpenResult(
            opened = leftShade,
            matchedText = matchedLabel,
            destinationPackage = destination,
            error = if (leftShade) null else "The notification was tapped, but the notification shade remained open.",
        )
    }
