...
object ScreenRecordHandler {
    ...
    fun handleScreenRecord(context: Context, op: JSONObject): OpResult {
        ...
        if (!isForeground(context)) {
            return OpResult(false, error = "FOREGROUND_REQUIRED: Screen recording requires the Jarvis Daemon app to be in the foreground. Open the Jarvis Daemon app and try again.")
        }
        ...
        if (captureIntent == null) {
            return OpResult(
                ...
                error = "SCREEN_RECORD_PERMISSION_REQUIRED: Screen recording requires a one-time permission from Android. " +
                    "Open the Jarvis Daemon app, tap 'Allow Screen Capture', approve the system prompt, then try again."
            )
        }
        ...
        try {
            ...
            projection = mpm.getMediaProjection(android.app.Activity.RESULT_OK, captureIntent)
                ?: return OpResult(false, error = "SCREEN_RECORD_PERMISSION_REQUIRED: Screen capture permission expired. Open the Jarvis Daemon app and re-grant screen recording.")
        } catch (e: Exception) {
            projectionIntent = null
            return OpResult(
                false,
                error = "Screen recording setup failed: ${e.message}. Open the Jarvis Daemon app and re-grant screen recording."
            )
        }
