...
object CameraHandler {
    ...
    fun handleSnap(context: Context, op: JSONObject): OpResult {
        if (!isForeground(context)) {
            return OpResult(false, error = "FOREGROUND_REQUIRED: Camera capture requires the Jarvis Daemon app to be in the foreground. Open the Jarvis Daemon app and try again.")
        }
        if (!hasPermission(context, Manifest.permission.CAMERA)) {
            return OpResult(
                false,
                error = "CAMERA_PERMISSION_REQUIRED: Jarvis cannot access the camera. " +
                    "Open the Jarvis Daemon app, tap 'Grant Camera', or go to Settings → Apps → Jarvis Daemon → Permissions → Camera and enable it."
            )
        }
    ...
    fun handleClip(context: Context, op: JSONObject): OpResult {
        if (!isForeground(context)) {
            return OpResult(false, error = "FOREGROUND_REQUIRED: Video recording requires the Jarvis Daemon app to be in the foreground. Open the Jarvis Daemon app and try again.")
        }
        if (!hasPermission(context, Manifest.permission.CAMERA)) {
            return OpResult(
                false,
                error = "CAMERA_PERMISSION_REQUIRED: Camera permission is required for video recording. " +
                    "Go to Settings → Apps → Jarvis Daemon → Permissions → Camera and enable it."
            )
        }
        ...
        if (audioEnabled && !hasPermission(context, Manifest.permission.RECORD_AUDIO)) {
            return OpResult(
                ...
                error = "MICROPHONE_PERMISSION_REQUIRED: Audio recording requires microphone permission. " +
                    "Disable audio or grant the microphone permission to Jarvis Daemon in Settings."
            )
        }
