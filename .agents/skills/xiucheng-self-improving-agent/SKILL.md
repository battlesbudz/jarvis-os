...
## Project-Specific Usage Note

The `SelfImprovingAgent` class defaults its workspace path to `/root/.openclaw/workspace`. In this project, `SOUL.md` and `improvement_log.md` live at the **repo root** (`/home/runner/workspace`). Always pass the repo root explicitly when instantiating:

```python
import os
from self_improving import SelfImprovingAgent

sia = SelfImprovingAgent(workspace=os.getcwd())
```

This ensures the skill reads and writes the correct files (`./SOUL.md`, `./improvement_log.md`) rather than the OpenClaw default workspace path.
